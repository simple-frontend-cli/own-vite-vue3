...
<template>
    <ul class="nav-menu-ul" :class="open ? 'nav-open-ul' : 'nav-close-ul'" v-if="data.length" :id="parentId">
        <li v-for="(item, index) in data" :key="index">
            <div class="menu-operate-div" @click="handleMenu(item)">
                ...
                <span :class="item.title === CurItem ? 'normal-menu active-menu' : 'normal-menu'">
                    {{ item.title }}
                </span>
                <span class="menu-expand-span" v-if="item?.children && item?.children.length">
                    <caret-forward v-if="OpenMenu.indexOf(item.name)" />
                    <caret-down v-else />
                </span>
            </div>
            <menu-component :data="item?.children || []" :parentId="item.name" :open="false" />
        </li>
    </ul>
</template>
...
.nav-open-ul {
    animation: open-menu-animation .5s ease;
}
